...
if (isset($_GET['id'])) {
    ...
    $payload = json_encode(['ids' => $userIds]);
    if ($payload === false) {
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "http://localhost:3003/users/admin");
    ...
    if ($response) {
        ...
        if ($httpCode === 200 || ($data['stateCode'] ?? null) === 200) {
            echo "<script>alert('Xoá người dùng thành công!'); window.location.href='/testphp/admin/index.php?action=users';</script>";
            exit;
        } else {
            echo "<script>alert('Lỗi khi xoá người dùng: " . ($data['msg'] ?? 'Không rõ') . " (Mã lỗi: $httpCode)'); window.history.back();</script>";
            exit;
        }
    } else {
