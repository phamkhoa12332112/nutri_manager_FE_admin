...
if (isset($_GET['id'])) {
    ...
    $payload = json_encode(['ids' => $ingredientId]);
    if ($payload === false) {
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "http://localhost:3003/ingredients");
    ...
    if ($response) {
        ...
        if ($httpCode === 200 || ($data['stateCode'] ?? null) === 200) {
            echo "<script>alert('Xoá nguyên liệu thành công!'); wi  ndow.location.href='/testphp/admin/index.php?action=ingredients';</script>";
            exit;
        } else {
            echo "<script>alert('Lỗi khi xoá nguyên liệu: " . ($data['msg'] ?? 'Không rõ') . " (Mã lỗi: $httpCode)'); window.history.back();</script>";
            exit;
        }
    } else {
