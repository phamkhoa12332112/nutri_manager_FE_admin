...
if (isset($_GET['id'])) {
    ...
    $payload = json_encode(['ids' => $moodIds]);
    if ($payload === false) {
    ...
    if ($response) {
        ...
        if ($httpCode === 200 || ($data['stateCode'] ?? null) === 200) {
            echo "<script>alert('Xoá tâm trạng thành công!'); window.location.href='/testphp/admin/index.php?action=moods';</script>";
            exit;
        } else {
            echo "<script>alert('Lỗi khi xoá tâm trạng: " . ($data['msg'] ?? 'Không rõ') . " (Mã lỗi: $httpCode)'); window.history.back();</script>";
            exit;
        }
    } else {
